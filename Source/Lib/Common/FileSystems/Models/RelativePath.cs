...
public struct RelativePath
{
        ...
        IFileSystemProvider fileSystemProvider)
    {
        ...
        // UpDirDirectiveCount
        {
            ...
            var moveUpDirectoryToken = $"..{fileSystemProvider.DirectorySeparatorChar}";
        ...
        if (UpDirDirectiveCount == 0)
        {
            ...
            var currentDirectoryToken = $".{fileSystemProvider.DirectorySeparatorChar}";
        ...
        if (IsDirectory)
        {
            ...
            if (fileSystemProvider.IsDirectorySeparator(relativePathString.LastOrDefault()))
        ...
        while (_position < relativePathString.Length)
        {
            ...
            if (fileSystemProvider.IsDirectorySeparator(currentCharacter))
                ConsumeTokenAsDirectory(fileSystemProvider);
            else
        ...
        else
        {
            ...
            ExtensionNoPeriod = fileSystemProvider.DirectorySeparatorChar.ToString();
        }
    }
    ...
    private void ConsumeTokenAsDirectory(IFileSystemProvider fileSystemProvider)
    {
        var nameNoExtension = _tokenBuilder.ToString();
        var nameWithExtension = nameNoExtension + fileSystemProvider.DirectorySeparatorChar;

        _parentDirectoriesBuilder.Append(nameWithExtension);

        AncestorDirectoryList.Add((nameNoExtension + fileSystemProvider.DirectorySeparatorChar, _parentDirectoriesBuilder.ToString()));

        _tokenBuilder.Clear();
    }

    private string CalculateValue(IFileSystemProvider fileSystemProvider)
    {
        if (_value is null)
        {
            ...
            if (relativePathString.EndsWith(new string(fileSystemProvider.DirectorySeparatorChar, 2)) ||
                relativePathString.EndsWith(new string(fileSystemProvider.AltDirectorySeparatorChar, 2)))
            {
                relativePathString = relativePathString[..^1];
            }
