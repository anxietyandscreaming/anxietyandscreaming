...
namespace Clair.Ide.RazorLib.AppDatas.Models;
...
public class NativeAppDataService : IAppDataService
{
        ...
        where AppData : IAppData
    {
        if (!_isInitialized)
        {
            ...
            var directoryPath = _commonService.FileSystemProvider.SafeRoamingApplicationDataDirectoryAbsolutePath.Value;
    ...
    public string GetFilePath(string assemblyName, string typeName, string uniqueIdentifier)
    {
        ...
        return _commonService.FileSystemProvider.JoinPaths(
            _commonService.FileSystemProvider.SafeRoamingApplicationDataDirectoryAbsolutePath.Value,
            relativePath);
    }
}
