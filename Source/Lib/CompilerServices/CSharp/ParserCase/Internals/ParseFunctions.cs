...
public static partial class Parser
{
    ...
    {
        var functionDefinitionNode = parserModel.Rent_FunctionDefinitionNode();        functionDefinitionNode.ReturnTypeReference = consumedTypeReference;
        functionDefinitionNode.FunctionIdentifierToken = consumedIdentifierToken;
        functionDefinitionNode.AbsolutePathId = parserModel.AbsolutePathId;
        ...
        }*/        parserModel.RegisterScope(            new Scope(                ScopeDirectionKind.Down,                scope_StartInclusiveIndex: parserModel.TokenWalker.Current.TextSpan.StartInclusiveIndex,                scope_EndExclusiveIndex: -1,                codeBlock_StartInclusiveIndex: -1,                codeBlock_EndExclusiveIndex: -1,                parentScopeSubIndex: parserModel.ScopeCurrentSubIndex,                selfScopeSubIndex: parserModel.Compilation.ScopeLength,                nodeSubIndex: parserModel.Compilation.NodeLength,                permitCodeBlockParsing: true,                isImplicitOpenCodeBlockTextSpan: false,                ownerSyntaxKind: functionDefinitionNode.SyntaxKind),            functionDefinitionNode);        if (parserModel.TokenWalker.Current.SyntaxKind == SyntaxKind.OpenAngleBracketToken)
        {
            ...
            _ = Parser.TryParseExpression(ref parserModel, out _);
        ...
        if (functionDefinitionNode.TraitsIndex != -1)
        {
            var traits = parserModel.Binder.FunctionDefinitionTraitsList[functionDefinitionNode.TraitsIndex];            traits.OffsetFunctionArgumentEntryList = functionDefinitionNode.OffsetFunctionArgumentEntryList;            traits.LengthFunctionArgumentEntryList = functionDefinitionNode.LengthFunctionArgumentEntryList;            parserModel.Binder.FunctionDefinitionTraitsList[functionDefinitionNode.TraitsIndex] = traits;
        ...
        parserModel.Return_FunctionDefinitionNode(functionDefinitionNode);        if (parserModel.TokenWalker.Current.SyntaxKind == SyntaxKind.StatementDelimiterToken)
            ...
            Parser.MoveToExpressionBody(ref parserModel);
        ...
        ref CSharpParserState parserModel)
    {
        ...
        var constructorDefinitionNode = parserModel.Rent_ConstructorDefinitionNode();        constructorDefinitionNode.ReturnTypeReference = new TypeReferenceValue(typeClauseNode);        constructorDefinitionNode.FunctionIdentifier = consumedIdentifierToken;        constructorDefinitionNode.AbsolutePathId = parserModel.AbsolutePathId;        parserModel.Return_TypeClauseNode(typeClauseNode);        parserModel.BindConstructorDefinitionIdentifierToken(consumedIdentifierToken);        parserModel.RegisterScope(            new Scope(                ScopeDirectionKind.Down,                scope_StartInclusiveIndex: parserModel.TokenWalker.Current.TextSpan.StartInclusiveIndex,                scope_EndExclusiveIndex: -1,                codeBlock_StartInclusiveIndex: -1,                codeBlock_EndExclusiveIndex: -1,                parentScopeSubIndex: parserModel.ScopeCurrentSubIndex,                selfScopeSubIndex: parserModel.Compilation.ScopeLength,                nodeSubIndex: parserModel.Compilation.NodeLength,                permitCodeBlockParsing: true,                isImplicitOpenCodeBlockTextSpan: false,                ownerSyntaxKind: constructorDefinitionNode.SyntaxKind),            constructorDefinitionNode);        HandleFunctionArguments(constructorDefinitionNode, ref parserModel);

        parserModel.Return_ConstructorDefinitionNode(constructorDefinitionNode);
        ...
        VariableKind variableKind = VariableKind.Local)
    {
        ...
        var offsetFunctionArgumentEntryList = parserModel.Binder.FunctionArgumentList.Count;
        var lengthFunctionArgumentEntryList = 0;
        ...
        var corruptState = false;        while (!parserModel.TokenWalker.IsEof)
        {
            ...
            else if (!corruptState)
            {
                ...
                }                var tokenIndexOriginal = parserModel.TokenWalker.Index;                var successParse = Parser.TryParseVariableDeclarationNode(ref parserModel, out var variableDeclarationNode);                if (successParse)
                {
                    ...
                    }                    parserModel.Binder.FunctionArgumentList.Add(
                        new FunctionArgument(
                            variableDeclarationNode,
                            optionalCompileTimeConstantToken,
                            parserModel.ArgumentModifierKind));
                    lengthFunctionArgumentEntryList++;
        ...
        functionDefinitionNode.OffsetFunctionArgumentEntryList = offsetFunctionArgumentEntryList;
        functionDefinitionNode.LengthFunctionArgumentEntryList = lengthFunctionArgumentEntryList;
