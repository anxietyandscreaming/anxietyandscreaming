...
namespace Clair.Common.RazorLib.Reactives.Models;
...
public class ProgressBarModel : IDisposable
{
    ...
    {
        if (IsDisposed)
            throw new ClairCommonException($"The {nameof(ProgressBarModel)} has {nameof(IsDisposed)} set to true, therefore cannot be set anymore.");
